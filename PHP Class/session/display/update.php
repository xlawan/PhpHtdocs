<?php
	...
	if(isset($_POST["update"]))
	{
		...
		if($description2 == "" || $umsr2 == "" || $category2 == "" || $qty2 == "" || $price2 == "")
		{
			...
			$qty = $_POST["qtyonhand2"];
			$price = $_POST["price2"];	
		}
		else if(!is_numeric($qty2) || !is_numeric($price2))
		{
			echo "<p>Sorry, Quantity on Hand or Price must be a number.</p>";
			//exit;
			//retainInputs();
			$itemid = $_POST["iid2"];
			$description = $_POST["description2"];
		...
		{
			echo "<p>Sorry, Quantity on Hand or Price must not be less than to '0'.</p>";
			//exit;
			$itemid = $_POST["iid2"];
			$description = $_POST["description2"];
			$umsr = $_POST["umsr2"];
